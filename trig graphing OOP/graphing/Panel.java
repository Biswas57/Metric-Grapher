...
public class Panel extends JPanel {
    ...
    private NewStatistics strategy = new AllPoints();
    ...
    public void setStrategy(NewStatistics strategy) {
    ...
    private void paintData(Graphics2D g) {
        List<Double> emitterData = strategy.compute(plot.getData());

        double x = (emitterData.size() > 1) ? (double) (getWidth() - 2 * marg) / (emitterData.size() - 1)
                : getWidth() / 2.0 - marg;
